...
{//eine komplette Löschung erfolgt nur, wenn der Absender die Nachricht bereits bei sich gelöscht hat, ansosnten wird ihr Status lediglich auf "für den Empfänger gelöscht>" gesetzt. 

$daten = mysql_fetch_array($ergebnis);$Empf_Status=$daten["eing"];$Abs_Status=$daten["ausg"];echo $Empf_Status;echo $Abs_Status;if($Empf_Status==1 and $Abs_Status==0){
$sql = "DELETE FROM `message` WHERE `message`.`pn_id` = $pnid"; $loeschen=mysql_query("DELETE FROM message WHERE empf_id='$uid' AND pn_id='$pnid'");
 echo $loeschen; 
}//ENDE ENTGÜLTIGES LÖSCHENelseif($Empf_Status==1 and $Abs_Status==1){$sql="UPDATE  `message` SET `message`.`eing`='0' WHERE `message`.`pn_id` = $pnid";//UPDATE `message` SET `eing` = '0' WHERE `message`.`pn_id` = 1$ausfuehren=mysql_query($sql);echo $sql;}//ENDE UPDATE STATUS

}//ENDE KOMPLETTE LÖSCHUNGSABFRAGE

}//ENDE DELETE}//Hier endet der Bereich "Aktion"
